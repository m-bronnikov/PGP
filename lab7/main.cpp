...
double u_next(double ux0, double ux1, double uy0, double uy1, 
                        ...
                        double h2y, double h2z, double denum){
...
int main(int argc, char **argv){
    ...
    // compute steps:
    double h2x, h2y, h2z, denuminator; 
    ...
    h2y *= h2y;
    h2z *= h2z;
    denuminator = 2.0*(1.0/h2x + 1.0/h2y + 1.0/h2z);
    ...
    auto u_ijk = [buffer0, blocks, edge_buff_in, idx, edge_idx](int i, int j, int k){
        if(i < 0){
            return edge_buff_in[left][edge_idx(j, k, blocks[dir_y])];
        }
        if(j < 0){
            return edge_buff_in[front][edge_idx(i, k, blocks[dir_x])];
        }
        if(k < 0){
            return edge_buff_in[down][edge_idx(i, j, blocks[dir_x])];
        }
        if(i == blocks[dir_x]){
            return edge_buff_in[right][edge_idx(j, k, blocks[dir_y])];
        }
        if(j == blocks[dir_y]){
            return edge_buff_in[back][edge_idx(i, k, blocks[dir_x])];
        }
        if(k == blocks[dir_z]){
            return edge_buff_in[up][edge_idx(i, j, blocks[dir_x])];
        }
        return buffer0[idx(i, j, k)];
    };
    ...
    do{
        ...
        // 3.1 left:
        if(coords[dir_x]){
            recv_waiting(&in[left], &out[left]);
        }
        if(coords[dir_x] < dimens[dir_x] - 1){
            recv_waiting(&in[right], &out[right]);
        }
            ...
            for(int k = 1; k < blocks[dir_z] - 1; ++k){
                for(int j = 1; j < blocks[dir_y] - 1; ++j){
                    for(int orntr = left; orntr <= right; ++orntr){
                        // compute value in main buffer
                        int addition = (orntr & 1) ? -1 : 1;
                        int ii = (orntr & 1) ? blocks[dir_x] - 1 : 0;

                        buffer1[idx(ii, j, k)] = u_next(
                            u_ijk(ii - addition, j, k), u_ijk(ii + addition, j, k),
        ...
        // 3.2 front:
        if(coords[dir_y]){
            recv_waiting(&in[front], &out[front]); // may be moved into the loop
        }
        if(coords[dir_y] < dimens[dir_y] - 1){
            recv_waiting(&in[back], &out[back]);
        }

        {
            for(int k = 1; k < blocks[dir_z] - 1; ++k){
                for(int orntr = front; orntr <= back; ++orntr){
                    int addition = (orntr & 1) ? -1 : 1; // may be removed
                    int jj = (orntr & 1) ? blocks[dir_y] - 1 : 0;
                    for(int i = 1; i < blocks[dir_x] - 1; ++i){
                        buffer1[idx(i, jj, k)] = u_next(
                            buffer0[idx(i - 1, jj, k)], buffer0[idx(i + 1, jj, k)],
                            u_ijk(i, jj - addition, k), u_ijk(i, jj + addition, k),
        ...
        // 3.3 down:
        if(coords[dir_z]){
            recv_waiting(&in[down], &out[down]);
        }
        if(coords[dir_z] < dimens[dir_z] - 1){
            recv_waiting(&in[up], &out[up]);
        }
                ...
                for(int j = 1; j < blocks[dir_y] - 1; ++j){
                    for(int i = 1; i < blocks[dir_x] - 1; ++i){
                        buffer1[idx(i, j, kk)] = u_next(
                            ...
                            u_ijk(i, j, kk - addition), u_ijk(i, j, kk + addition),
                            h2x, h2y, h2z, denuminator
                        );
        ...
        for(int o_k = down; o_k <= up; ++o_k){
            ...
            for(int o_j = front; o_j <= back; ++o_j){
                ...
                for(int i = 0; i < blocks[dir_x]; ++i){
                    ...
                    buffer1[idx(i, j, k)] = u_next(ux1, ux2, uy1, uy2, uz1, uz2, h2x, h2y, h2z, denuminator);
    ...
    MPI_Barrier(grid_comm); // stop before print (may be removed)

    #ifdef DEBUG
    if(main_worker){
        cout << "||Search after #1||" << endl;
        cout << std::flush;
    }
    #endif
